...
def test_build_fba_fee_preview_merge_sql_uses_business_key_hash() -> None:
    sql = build_fba_fee_preview_merge_sql(table_spec=FBA_FEE_PREVIEW_TARGET_TABLE_SPEC)

    assert "MERGE dbo.[amazon_fba_fee_preview]" in sql
...
def test_fba_fee_preview_insert_sql_rejects_unknown_table() -> None:
    try:
...
def test_fba_fee_preview_repo_upsert_counts_merge_actions() -> None:
    cursor = FakeCursor(fetch_values=[["INSERT"], ["UPDATE"]])
    repo = FbaFeePreviewRepo(FakeConnection(cursor))
    row = {column: None for column in FBA_FEE_PREVIEW_TARGET_TABLE_SPEC.table_columns}
    row.update(
        {
            "marketplace_id": "ATVPDKIKX0DER",
            "source_report_type": "GET_FBA_ESTIMATED_FBA_FEES_TXT_DATA",
    ...
    result = repo.upsert_fba_fee_preview_rows(rows=[row, row2], source_run_id=42)
...
def test_fba_fee_preview_repo_insert_and_update_sync_run_log() -> None:
