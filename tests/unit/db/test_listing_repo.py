...
from seller_data_pipeline.ingestion.listing_table_mapping import LISTING_TARGET_TABLE_SPEC
...
def test_build_listing_merge_sql_uses_business_key_hash() -> None:
    sql = build_listing_merge_sql(table_spec=LISTING_TARGET_TABLE_SPEC)

    assert "MERGE dbo.[amazon_listing_snapshot]" in sql
...
def test_listing_insert_sql_rejects_unknown_table() -> None:
    try:
...
def test_listing_repo_upsert_counts_merge_actions() -> None:
    cursor = FakeCursor(fetch_values=[["INSERT"], ["UPDATE"]])
    repo = ListingRepo(FakeConnection(cursor))
    row = {column: None for column in LISTING_TARGET_TABLE_SPEC.table_columns}
    row.update(
        {
    ...
    result = repo.upsert_listing_rows(rows=[row, row2], source_run_id=42)
    ...
    assert cursor.executed[0][1][LISTING_TARGET_TABLE_SPEC.table_columns.index("source_run_id")] == 42


def test_listing_repo_insert_and_update_sync_run_log() -> None:
