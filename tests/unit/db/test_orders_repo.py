...
from seller_data_pipeline.db.repositories.orders_repo import (
    OrdersRepo,
    build_insert_sql,
    build_orders_merge_sql,
)
from seller_data_pipeline.ingestion.orders_table_mapping import ORDERS_TARGET_TABLE_SPEC
...
def test_build_orders_merge_sql_uses_business_key_hash() -> None:
    sql = build_orders_merge_sql(table_spec=ORDERS_TARGET_TABLE_SPEC)

    assert "MERGE dbo.[amazon_order_item]" in sql
...
def test_orders_insert_sql_rejects_unknown_table() -> None:
    try:
...
def test_orders_repo_upsert_counts_merge_actions() -> None:
    cursor = FakeCursor(fetch_values=[["INSERT"], ["UPDATE"]])
    repo = OrdersRepo(FakeConnection(cursor))
    row = {column: None for column in ORDERS_TARGET_TABLE_SPEC.table_columns}
    row.update(
        {
            "marketplace_id": "ATVPDKIKX0DER",
            "source_report_type": "GET_FLAT_FILE_ALL_ORDERS_DATA_BY_ORDER_DATE_GENERAL",
    ...
    result = repo.upsert_order_item_rows(rows=[row, row2], source_run_id=42)

    assert result.attempted_rows == 2
    assert result.inserted_rows == 1
    assert result.updated_rows == 1
    assert len(cursor.executed) == 2
    assert cursor.executed[0][1][ORDERS_TARGET_TABLE_SPEC.table_columns.index("source_run_id")] == 42


def test_orders_repo_insert_and_update_sync_run_log() -> None:
