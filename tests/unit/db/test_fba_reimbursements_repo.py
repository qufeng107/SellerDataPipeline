...
def test_build_fba_reimbursements_merge_sql_uses_business_key_hash() -> None:
    sql = build_fba_reimbursements_merge_sql(table_spec=FBA_REIMBURSEMENTS_TARGET_TABLE_SPEC)

    assert "MERGE dbo.[amazon_fba_reimbursement]" in sql
...
def test_fba_reimbursements_insert_sql_rejects_unknown_table() -> None:
    try:
...
def test_fba_reimbursements_repo_upsert_counts_merge_actions() -> None:
    cursor = FakeCursor(fetch_values=[["INSERT"], ["UPDATE"]])
    repo = FbaReimbursementsRepo(FakeConnection(cursor))
    row = {column: None for column in FBA_REIMBURSEMENTS_TARGET_TABLE_SPEC.table_columns}
    row.update(
        {
            "marketplace_id": "ATVPDKIKX0DER",
            "source_report_type": "GET_FBA_REIMBURSEMENTS_DATA",
    ...
    result = repo.upsert_fba_reimbursement_rows(rows=[row, row2], source_run_id=42)

    assert result.attempted_rows == 2
    assert result.inserted_rows == 1
    assert result.updated_rows == 1
    assert len(cursor.executed) == 2
    source_run_id_index = FBA_REIMBURSEMENTS_TARGET_TABLE_SPEC.table_columns.index("source_run_id")
    assert cursor.executed[0][1][source_run_id_index] == 42


def test_fba_reimbursements_repo_insert_and_update_sync_run_log() -> None:
