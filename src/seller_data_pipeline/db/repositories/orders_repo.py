...
@dataclass(frozen=True)
class OrdersUpsertRunResult:
    table_result: OrdersUpsertTableResult
...
class OrdersRepo:
    """Azure SQL repository for Orders normalized table."""
    ...
    def upsert_order_item_rows(
        self,
        *,
        rows: list[dict[str, Any]],
        source_run_id: int | None = None,
        table_spec: OrdersTargetTableSpec = ORDERS_TARGET_TABLE_SPEC,
    ) -> OrdersUpsertTableResult:
        validate_orders_table_spec(table_spec)
        sql = build_orders_merge_sql(table_spec=table_spec)
        cursor = self.connection.cursor()
        inserted = 0
        updated = 0
        skipped = 0
        ...
        for row in rows:
            ...
            if action == "INSERT":
                ...
                updated += 1
        return OrdersUpsertTableResult(
...
class NullOrdersRepo:
    ...
    def upsert_order_item_rows(
        ...
        *,
        ...
        source_run_id: int | None = None,
        table_spec: OrdersTargetTableSpec = ORDERS_TARGET_TABLE_SPEC,
    ) -> OrdersUpsertTableResult:  # noqa: ARG002
        return OrdersUpsertTableResult(
            table_name=table_spec.target_table,
            report_type=table_spec.report_type,
            attempted_rows=len(rows),
...
def validate_orders_table_spec(table_spec: OrdersTargetTableSpec) -> None:
    ...
    for required_column in ("source_row_index", "business_key_hash"):
        if required_column not in table_spec.table_columns:
            raise ValueError(
                f"Orders target table lacks {required_column}: {table_spec.target_table}"
            )


def build_orders_merge_sql(*, table_spec: OrdersTargetTableSpec) -> str:
    validate_orders_table_spec(table_spec)
    columns = table_spec.table_columns
...
def validate_static_table_name(table_name: str) -> None:
    if table_name not in _ALLOWED_TABLES:
        raise ValueError(f"Table is not allowlisted for Orders repository writes: {table_name}")
