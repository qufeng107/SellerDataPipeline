...
_ALLOWED_TABLES = {spec.target_table for spec in PROMOTION_COUPON_TARGET_TABLE_SPECS} | {
    "amazon_sync_run_log",
...
class PromotionCouponUpsertTableResult:
    table_name: str
    report_type: str
    attempted_rows: int
    inserted_rows: int
...
@dataclass(frozen=True)
class PromotionCouponUpsertRunResult:
    table_results: tuple[PromotionCouponUpsertTableResult, ...]
...
class PromotionCouponRepo:
    """Azure SQL repository for Promotion/Coupon normalized tables."""
    ...
    def upsert_rows(
        ...
        *,
        ...
        validate_promotion_coupon_table_spec(table_spec)
        sql = build_promotion_coupon_merge_sql(table_spec=table_spec)
        cursor = self.connection.cursor()
        inserted = 0
        updated = 0
        skipped = 0
        ...
        for row in rows:
            ...
            if action == "INSERT":
                ...
                updated += 1
        return PromotionCouponUpsertTableResult(
...
class NullPromotionCouponRepo:
    ...
    def upsert_rows(
        ...
        *,
        ...
        rows: list[dict[str, Any]],
        source_run_id: int | None = None,
    ) -> PromotionCouponUpsertTableResult:  # noqa: ARG002
        return PromotionCouponUpsertTableResult(
            table_name=table_spec.target_table,
            report_type=table_spec.report_type,
            attempted_rows=len(rows),
...
def validate_promotion_coupon_table_spec(table_spec: PromotionCouponTargetTableSpec) -> None:
    ...
    for required_column in ("source_row_index", "business_key_hash"):
        if required_column not in table_spec.table_columns:
            raise ValueError(
                f"Promotion/Coupon target table lacks {required_column}: {table_spec.target_table}"
            )


def build_promotion_coupon_merge_sql(*, table_spec: PromotionCouponTargetTableSpec) -> str:
    validate_promotion_coupon_table_spec(table_spec)
    columns = table_spec.table_columns
