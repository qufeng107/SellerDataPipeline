...
class FbaFeePreviewUpsertTableResult:
    table_name: str
    report_type: str
    attempted_rows: int
    inserted_rows: int
...
@dataclass(frozen=True)
class FbaFeePreviewUpsertRunResult:
    table_result: FbaFeePreviewUpsertTableResult
...
class FbaFeePreviewRepo:
    """Azure SQL repository for FbaFeePreview normalized table."""
    ...
    def upsert_fba_fee_preview_rows(
        self,
        *,
        rows: list[dict[str, Any]],
        source_run_id: int | None = None,
        table_spec: FbaFeePreviewTargetTableSpec = FBA_FEE_PREVIEW_TARGET_TABLE_SPEC,
    ) -> FbaFeePreviewUpsertTableResult:
        validate_fba_fee_preview_table_spec(table_spec)
        sql = build_fba_fee_preview_merge_sql(table_spec=table_spec)
        cursor = self.connection.cursor()
        inserted = 0
        updated = 0
        skipped = 0
        ...
        for row in rows:
            ...
            if action == "INSERT":
                ...
                updated += 1
        return FbaFeePreviewUpsertTableResult(
...
class NullFbaFeePreviewRepo:
    ...
    def upsert_fba_fee_preview_rows(
        ...
        *,
        ...
        source_run_id: int | None = None,
        table_spec: FbaFeePreviewTargetTableSpec = FBA_FEE_PREVIEW_TARGET_TABLE_SPEC,
    ) -> FbaFeePreviewUpsertTableResult:  # noqa: ARG002
        return FbaFeePreviewUpsertTableResult(
            table_name=table_spec.target_table,
            report_type=table_spec.report_type,
            attempted_rows=len(rows),
...
def validate_fba_fee_preview_table_spec(table_spec: FbaFeePreviewTargetTableSpec) -> None:
    if table_spec.target_table != FBA_FEE_PREVIEW_TARGET_TABLE:
        raise ValueError(
            f"FbaFeePreview target table is not allowlisted: {table_spec.target_table}"
        )
    for required_column in ("source_row_index", "business_key_hash"):
        if required_column not in table_spec.table_columns:
            raise ValueError(
                f"FbaFeePreview target table lacks {required_column}: {table_spec.target_table}"
            )


def build_fba_fee_preview_merge_sql(*, table_spec: FbaFeePreviewTargetTableSpec) -> str:
    validate_fba_fee_preview_table_spec(table_spec)
    columns = table_spec.table_columns
...
def validate_static_table_name(table_name: str) -> None:
    if table_name not in _ALLOWED_TABLES:
        raise ValueError(
            f"Table is not allowlisted for FbaFeePreview repository writes: {table_name}"
        )
...
__all__ = [
    "NullFbaFeePreviewRepo",
    "FbaFeePreviewRepo",
    "FbaFeePreviewUpsertRunResult",
    "FbaFeePreviewUpsertTableResult",
    "build_insert_sql",
    "build_insert_with_output_sql",
    "build_fba_fee_preview_merge_sql",
    "validate_fba_fee_preview_table_spec",
]
