...
class FbaReimbursementsUpsertTableResult:
    table_name: str
    report_type: str
    attempted_rows: int
    inserted_rows: int
...
@dataclass(frozen=True)
class FbaReimbursementsUpsertRunResult:
    table_result: FbaReimbursementsUpsertTableResult
...
class FbaReimbursementsRepo:
    """Azure SQL repository for FbaReimbursements normalized table."""
    ...
    def upsert_fba_reimbursement_rows(
        self,
        *,
        rows: list[dict[str, Any]],
        source_run_id: int | None = None,
        table_spec: FbaReimbursementsTargetTableSpec = FBA_REIMBURSEMENTS_TARGET_TABLE_SPEC,
    ) -> FbaReimbursementsUpsertTableResult:
        validate_fba_reimbursements_table_spec(table_spec)
        sql = build_fba_reimbursements_merge_sql(table_spec=table_spec)
        cursor = self.connection.cursor()
        inserted = 0
        updated = 0
        skipped = 0
        ...
        for row in rows:
            ...
            if action == "INSERT":
                ...
                updated += 1
        return FbaReimbursementsUpsertTableResult(
...
class NullFbaReimbursementsRepo:
    ...
    def upsert_fba_reimbursement_rows(
        ...
        *,
        ...
        source_run_id: int | None = None,
        table_spec: FbaReimbursementsTargetTableSpec = FBA_REIMBURSEMENTS_TARGET_TABLE_SPEC,
    ) -> FbaReimbursementsUpsertTableResult:  # noqa: ARG002
        return FbaReimbursementsUpsertTableResult(
            table_name=table_spec.target_table,
            report_type=table_spec.report_type,
            attempted_rows=len(rows),
...
def validate_fba_reimbursements_table_spec(table_spec: FbaReimbursementsTargetTableSpec) -> None:
    if table_spec.target_table != FBA_REIMBURSEMENTS_TARGET_TABLE:
        raise ValueError(
            f"FbaReimbursements target table is not allowlisted: {table_spec.target_table}"
        )
    for required_column in ("source_row_index", "business_key_hash"):
        if required_column not in table_spec.table_columns:
            raise ValueError(
                f"FbaReimbursements target table lacks {required_column}: {table_spec.target_table}"
            )


def build_fba_reimbursements_merge_sql(*, table_spec: FbaReimbursementsTargetTableSpec) -> str:
    validate_fba_reimbursements_table_spec(table_spec)
    columns = table_spec.table_columns
...
def validate_static_table_name(table_name: str) -> None:
    if table_name not in _ALLOWED_TABLES:
        raise ValueError(
            f"Table is not allowlisted for FbaReimbursements repository writes: {table_name}"
        )
...
__all__ = [
    "NullFbaReimbursementsRepo",
    "FbaReimbursementsRepo",
    "FbaReimbursementsUpsertRunResult",
    "FbaReimbursementsUpsertTableResult",
    "build_insert_sql",
    "build_insert_with_output_sql",
    "build_fba_reimbursements_merge_sql",
    "validate_fba_reimbursements_table_spec",
]
