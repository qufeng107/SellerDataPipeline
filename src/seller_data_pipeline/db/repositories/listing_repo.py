...
class ListingUpsertTableResult:
    table_name: str
    report_type: str
    attempted_rows: int
    inserted_rows: int
...
class ListingRepo:
    """Azure SQL repository for amazon_listing_snapshot."""
    ...
    def upsert_listing_rows(
        self,
        *,
        rows: list[dict[str, Any]],
        source_run_id: int | None = None,
        table_spec: ListingTargetTableSpec = LISTING_TARGET_TABLE_SPEC,
    ) -> ListingUpsertTableResult:
        validate_listing_table_spec(table_spec)
        sql = build_listing_merge_sql(table_spec=table_spec)
        cursor = self.connection.cursor()
        inserted = 0
        updated = 0
        skipped = 0
        ...
        for row in rows:
            ...
            if action == "INSERT":
                ...
                updated += 1
        return ListingUpsertTableResult(
...
class NullListingRepo:
    ...
    def upsert_listing_rows(
        ...
        *,
        ...
        source_run_id: int | None = None,
        table_spec: ListingTargetTableSpec = LISTING_TARGET_TABLE_SPEC,
    ) -> ListingUpsertTableResult:  # noqa: ARG002
        return ListingUpsertTableResult(
            table_name=table_spec.target_table,
            report_type=table_spec.report_type,
            attempted_rows=len(rows),
...
def validate_listing_table_spec(table_spec: ListingTargetTableSpec) -> None:
    if table_spec.target_table != LISTING_TARGET_TABLE:
        raise ValueError(f"Listing target table is not allowlisted: {table_spec.target_table}")
    if "business_key_hash" not in table_spec.table_columns:
        raise ValueError(f"Listing target table lacks business_key_hash: {table_spec.target_table}")


def build_listing_merge_sql(*, table_spec: ListingTargetTableSpec) -> str:
    validate_listing_table_spec(table_spec)
    columns = table_spec.table_columns
...
def validate_static_table_name(table_name: str) -> None:
    if table_name not in _ALLOWED_TABLES:
        raise ValueError(f"Table is not allowlisted for Listing repository writes: {table_name}")
...
__all__ = [
    ...
    "build_listing_merge_sql",
    "validate_listing_table_spec",
]
