...
class FbaReimbursementsPreparedReportResult:
    report_type: str
    target_table: str
...
@dataclass(frozen=True)
class FbaReimbursementsIngestionDryRunResult:
    workflow_name: str
    marketplace_id: str
    started_at: str
    finished_at: str
    ...
    output_dir: str
    report_result: FbaReimbursementsPreparedReportResult
...
class FbaReimbursementsIngestionDryRunService:
    """Prepare DB-ready All FbaReimbursements rows locally without touching Azure SQL."""
    ...
    def __init__(
        ...
        *,
        ...
        self.parser = FbaReimbursementsParser()
    ...
    def prepare(
        ...
        *,
    ...
    ) -> FbaReimbursementsIngestionDryRunResult:
        ...
        prepared = self._prepare_fba_reimbursements_report(
        ...
        result = FbaReimbursementsIngestionDryRunResult(
            workflow_name="sp_api_fba_reimbursements_ingestion_dry_run",
            marketplace_id=marketplace_id,
            started_at=started_at,
            finished_at=finished_at,
            status=status,
        ...
        _write_json(run_output_dir / "fba_reimbursements_ingestion_summary.json", result.to_dict())
        return result

    def _prepare_fba_reimbursements_report(
        self,
        *,
        marketplace_id: str,
        raw_file_path: Path | None,
        preview_dir: Path,
    ) -> FbaReimbursementsPreparedReportResult:
        ...
        schema_result = validate_report_schema(
            analysis=analysis,
            expected_schema=build_fba_reimbursements_expected_schema(),
        )
        ...
        if schema_result.status in BLOCKING_SCHEMA_STATUSES or schema_result.requires_review:
            return FbaReimbursementsPreparedReportResult(
                report_type=FBA_REIMBURSEMENTS_REPORT_TYPE,
        ...
        rows = [
            map_fba_reimbursement_record_to_table_row(record, source_row_index=source_row_index)
        ...
        preview_path = (
            preview_dir / f"{FBA_REIMBURSEMENTS_TARGET_TABLE_SPEC.target_table}.preview.jsonl"
        )
...
def build_fba_reimbursements_expected_schema() -> ExpectedReportSchema:
    return ExpectedReportSchema(
        source_system="sp_api_reports",
        report_type=FBA_REIMBURSEMENTS_REPORT_TYPE,
        expected_fields=FBA_REIMBURSEMENTS_TARGET_TABLE_SPEC.expected_fields,
        required_fields=FBA_REIMBURSEMENTS_TARGET_TABLE_SPEC.required_fields,
        allow_extra_fields=False,
        allow_empty_report=False,
        notes="FBA reimbursements flat-file schema observed from GET_FBA_REIMBURSEMENTS_DATA.",
    )
...
def build_task_audit_event(
    *,
    ...
    return {
        "workflow_name": "sp_api_fba_reimbursements_ingestion_dry_run",
        "job_name": "prepare_fba_reimbursements_ingestion",
    ...
    payload.update(
        {
            ...
            "message": (
                "FBA Reimbursements dry-run detected duplicate business keys requiring "
                f"review: {reason}"
            ),
        }
    )
...
def _build_task_message(report_result: FbaReimbursementsPreparedReportResult) -> str:
    if report_result.requires_review:
        return f"Requires review: {FBA_REIMBURSEMENTS_REPORT_TYPE} ({report_result.skip_reason})"
    return f"Prepared FBA Reimbursements DB-ready preview rows: {report_result.prepared_row_count}"
...
__all__ = [
    ...
    "FbaReimbursementsPreparedReportResult",
    "build_fba_reimbursements_expected_schema",
