...
class InventoryPreparedReportResult:
    report_type: str
    target_table: str
...
@dataclass(frozen=True)
class InventoryIngestionDryRunResult:
    workflow_name: str
    marketplace_id: str
    started_at: str
    finished_at: str
    ...
    output_dir: str
    report_result: InventoryPreparedReportResult
...
class InventoryIngestionDryRunService:
    """Prepare DB-ready FBA inventory snapshot rows locally without touching Azure SQL."""
    ...
    def __init__(
        ...
        *,
        ...
        self.parser = FbaInventoryParser()
    ...
    def prepare(
        ...
        *,
        ...
        prepared = self._prepare_inventory_report(
        ...
        result = InventoryIngestionDryRunResult(
        ...
        _write_json(run_output_dir / "inventory_ingestion_summary.json", result.to_dict())
        return result

    def _prepare_inventory_report(
        self,
        *,
        marketplace_id: str,
        ...
        if path is None:
            return InventoryPreparedReportResult(
                report_type=INVENTORY_REPORT_TYPE,
        ...
        schema_result = validate_report_schema(
            analysis=analysis,
            expected_schema=build_inventory_expected_schema(),
        )
        ...
        rows = [map_inventory_record_to_table_row(record) for record in records]
        preview_path = preview_dir / f"{INVENTORY_TARGET_TABLE_SPEC.target_table}.preview.jsonl"
...
def build_inventory_expected_schema() -> ExpectedReportSchema:
    return ExpectedReportSchema(
        source_system="sp_api_reports",
        report_type=INVENTORY_REPORT_TYPE,
        expected_fields=INVENTORY_TARGET_TABLE_SPEC.expected_fields,
...
def _build_task_message(report_result: InventoryPreparedReportResult) -> str:
    if report_result.requires_review:
        return f"Requires review: {INVENTORY_REPORT_TYPE}"
    return f"Prepared Inventory DB-ready preview rows: {report_result.prepared_row_count}"
...
__all__ = [
    ...
    "InventoryPreparedReportResult",
    "build_inventory_expected_schema",
