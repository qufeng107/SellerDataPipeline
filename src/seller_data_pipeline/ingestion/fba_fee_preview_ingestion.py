...
from seller_data_pipeline.db.repositories.fba_fee_preview_repo import (
    FbaFeePreviewRepo,
    FbaFeePreviewUpsertRunResult,
)
...
from seller_data_pipeline.ingestion.fba_fee_preview_table_mapping import read_jsonl


@dataclass(frozen=True)
class FbaFeePreviewIngestionRunResult:
    workflow_name: str
    mode: str
    status: str
    requires_review: bool
    dry_run_result: FbaFeePreviewIngestionDryRunResult
    upsert_result: FbaFeePreviewUpsertRunResult | None
...
class FbaFeePreviewIngestionService:
    """Guarded FBA Fee Preview ingestion with dry-run default and explicit Azure SQL writes."""
    ...
    def __init__(
        ...
        *,
        ...
        self.dry_run_service = FbaFeePreviewIngestionDryRunService(
    ...
    def run(
        ...
        *,
    ...
    ) -> FbaFeePreviewIngestionRunResult:
        ...
        if dry_run_result.requires_review:
            return FbaFeePreviewIngestionRunResult(
                workflow_name="sp_api_fba_fee_preview_ingestion",
        ...
        if not execute:
            return FbaFeePreviewIngestionRunResult(
                ...
                sync_run_id=None,
                message=(
                    "FBA Fee Preview ingestion dry-run completed; "
        ...
        with get_connection() as conn:
            ...
            try:
                ...
                finished_event = build_finished_audit_event(
                    ...
                    started_at=started_at,
                    status="success",
                    message="SP-API FBA Fee Preview ingestion completed successfully.",
                )
                ...
                failed_event = build_finished_audit_event(
                    ...
                    started_at=started_at,
                    status="failed",
                    message="SP-API FBA Fee Preview ingestion failed and was rolled back.",
                    error=exc,
                )
        ...
        return FbaFeePreviewIngestionRunResult(
            ...
            requires_review=False,
            dry_run_result=dry_run_result,
            upsert_result=FbaFeePreviewUpsertRunResult(
                table_result=upsert_result.table_result,
                sync_run_id=sync_run_id,
            ),
            sync_run_id=sync_run_id,
            message="SP-API FBA Fee Preview ingestion completed and committed to Azure SQL.",
        )

    def _upsert_preview_rows(
        self,
        *,
        repo: FbaFeePreviewRepo,
        dry_run_result: FbaFeePreviewIngestionDryRunResult,
        source_run_id: int,
    ) -> FbaFeePreviewUpsertRunResult:
        preview_path = dry_run_result.report_result.preview_file_path
        rows = read_jsonl(preview_path) if preview_path else []
        table_result = repo.upsert_fba_fee_preview_rows(
            rows=rows,
            source_run_id=source_run_id,
        )
        return FbaFeePreviewUpsertRunResult(
    ...
    *,
    ...
    return {
        **dry_run_result.task_audit_event,
        "workflow_name": "sp_api_fba_fee_preview_ingestion",
        "job_name": "ingest_fba_fee_preview_report",
        ...
        "rows_skipped": 0,
        "rows_failed": 0,
        "message": "SP-API FBA Fee Preview ingestion is running.",
        "error_type": None,
        "error_detail": None,
    }
