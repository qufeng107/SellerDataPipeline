...
from seller_data_pipeline.ingestion.listing_table_mapping import read_jsonl


@dataclass(frozen=True)
class ListingIngestionRunResult:
    workflow_name: str
    mode: str
    status: str
    requires_review: bool
    dry_run_result: ListingIngestionDryRunResult
    upsert_result: ListingUpsertRunResult | None
...
class ListingIngestionService:
    """Guarded Listing ingestion workflow with dry-run default and explicit Azure SQL writes."""
    ...
    def __init__(
        ...
        *,
        ...
        self.dry_run_service = ListingIngestionDryRunService(
    ...
    def run(
        ...
        *,
        ...
        if dry_run_result.requires_review:
            return ListingIngestionRunResult(
                workflow_name="sp_api_listing_ingestion",
                ...
                upsert_result=None,
                sync_run_id=None,
                message="Listing ingestion requires schema review; Azure SQL write was blocked.",
            )
        if not execute:
            return ListingIngestionRunResult(
                workflow_name="sp_api_listing_ingestion",
                ...
                upsert_result=None,
                sync_run_id=None,
                message="Listing ingestion dry-run completed; no Azure SQL writes were performed.",
            )
        ...
        with get_connection() as conn:
            ...
            try:
                ...
                upsert_result = ListingUpsertRunResult(
                ...
                final_event = build_final_audit_event(
                    ...
                    started_at=started_at,
                    status="success",
                    message="SP-API Listing ingestion committed to Azure SQL.",
                )
        ...
        return ListingIngestionRunResult(
            ...
            message="SP-API Listing ingestion completed and committed to Azure SQL.",
        )
    ...
    def _upsert_preview_rows(
        ...
        *,
        ...
        if not preview_path:
            return repo.upsert_listing_rows(rows=[], source_run_id=source_run_id)
        rows = read_jsonl(preview_path)
        return repo.upsert_listing_rows(rows=rows, source_run_id=source_run_id)
...
def build_running_audit_event(
    *,
    ...
    event.update(
        {
            "workflow_name": "sp_api_listing_ingestion",
            "job_name": "ingest_listing_snapshot",
            ...
            "rows_written": 0,
            "rows_failed": 0,
            "message": "SP-API Listing ingestion is writing to Azure SQL.",
            "error_type": None,
            "error_detail": None,
        }
    )
    ...
    event.update(
        {
            ...
            "message": "SP-API Listing ingestion failed during Azure SQL upsert.",
