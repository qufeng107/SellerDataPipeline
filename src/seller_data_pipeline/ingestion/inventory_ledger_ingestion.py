...
class InventoryLedgerIngestionRunResult:
    workflow_name: str
    mode: str
    status: str
    requires_review: bool
    dry_run_result: InventoryLedgerIngestionDryRunResult
    upsert_result: InventoryLedgerUpsertRunResult | None
...
class InventoryLedgerIngestionService:
    """Guarded Inventory Ledger ingestion with dry-run default and explicit Azure SQL writes."""
    ...
    def __init__(
        ...
        *,
        ...
        self.dry_run_service = InventoryLedgerIngestionDryRunService(
        ...
        *,
    ...
    ) -> InventoryLedgerIngestionRunResult:
        dry_run_result = self.dry_run_service.prepare(
            marketplace_id=marketplace_id,
            summary_raw_file_path=summary_raw_file_path,
            detail_raw_file_path=detail_raw_file_path,
            fail_on_review=fail_on_review,
        )
        if dry_run_result.requires_review:
            return InventoryLedgerIngestionRunResult(
                workflow_name="sp_api_inventory_ledger_ingestion",
        ...
        if not execute:
            return InventoryLedgerIngestionRunResult(
                ...
                sync_run_id=None,
                message=(
                    "Inventory Ledger ingestion dry-run completed; "
        ...
        with get_connection(autocommit=False) as conn:
            ...
            try:
                ...
                finished_event = build_finished_audit_event(
                    ...
                    started_at=started_at,
                    status="success",
                    message="SP-API Inventory Ledger ingestion completed successfully.",
                )
                ...
                failed_event = build_finished_audit_event(
                    ...
                    started_at=started_at,
                    status="failed",
                    message="SP-API Inventory Ledger ingestion failed and was rolled back.",
                    error=exc,
                )
        ...
        return InventoryLedgerIngestionRunResult(
            ...
            requires_review=False,
            dry_run_result=dry_run_result,
            upsert_result=InventoryLedgerUpsertRunResult(
                table_results=upsert_result.table_results,
                sync_run_id=sync_run_id,
            ),
            sync_run_id=sync_run_id,
            message="SP-API Inventory Ledger ingestion completed and committed to Azure SQL.",
        )

    def _upsert_preview_rows(
        self,
        *,
        repo: InventoryLedgerRepo,
        dry_run_result: InventoryLedgerIngestionDryRunResult,
        source_run_id: int,
    ) -> InventoryLedgerUpsertRunResult:
        table_results: list[InventoryLedgerUpsertTableResult] = []
        for report_result in dry_run_result.report_results:
            ...
            for table_name, preview_path in report_result.preview_file_paths.items():
                table_spec = get_inventory_ledger_target_table_spec(table_name)
        ...
        return InventoryLedgerUpsertRunResult(
    ...
    *,
    ...
    return {
        **dry_run_result.task_audit_event,
        "workflow_name": "sp_api_inventory_ledger_ingestion",
        "job_name": "ingest_inventory_ledger_reports",
        ...
        "rows_skipped": 0,
        "rows_failed": 0,
        "message": "SP-API Inventory Ledger ingestion is running.",
        "error_type": None,
        "error_detail": None,
    }
