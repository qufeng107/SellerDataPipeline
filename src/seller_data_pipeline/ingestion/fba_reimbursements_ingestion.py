...
from seller_data_pipeline.db.repositories.fba_reimbursements_repo import (
    FbaReimbursementsRepo,
    FbaReimbursementsUpsertRunResult,
)
...
from seller_data_pipeline.ingestion.fba_reimbursements_table_mapping import read_jsonl


@dataclass(frozen=True)
class FbaReimbursementsIngestionRunResult:
    workflow_name: str
    mode: str
    status: str
    requires_review: bool
    dry_run_result: FbaReimbursementsIngestionDryRunResult
    upsert_result: FbaReimbursementsUpsertRunResult | None
...
class FbaReimbursementsIngestionService:
    """Guarded FBA Reimbursements ingestion with dry-run default and explicit Azure SQL writes."""
    ...
    def __init__(
        ...
        *,
        ...
        self.dry_run_service = FbaReimbursementsIngestionDryRunService(
    ...
    def run(
        ...
        *,
    ...
    ) -> FbaReimbursementsIngestionRunResult:
        ...
        if dry_run_result.requires_review:
            return FbaReimbursementsIngestionRunResult(
                workflow_name="sp_api_fba_reimbursements_ingestion",
                ...
                sync_run_id=None,
                message=(
                    "FBA Reimbursements ingestion requires schema review; "
        ...
        if not execute:
            return FbaReimbursementsIngestionRunResult(
                ...
                sync_run_id=None,
                message=(
                    "FBA Reimbursements ingestion dry-run completed; "
        ...
        with get_connection() as conn:
            repo = FbaReimbursementsRepo(conn)
            ...
            try:
                ...
                finished_event = build_finished_audit_event(
                    ...
                    started_at=started_at,
                    status="success",
                    message="SP-API FBA Reimbursements ingestion completed successfully.",
                )
                ...
                failed_event = build_finished_audit_event(
                    ...
                    started_at=started_at,
                    status="failed",
                    message="SP-API FBA Reimbursements ingestion failed and was rolled back.",
                    error=exc,
                )
        ...
        return FbaReimbursementsIngestionRunResult(
            ...
            requires_review=False,
            dry_run_result=dry_run_result,
            upsert_result=FbaReimbursementsUpsertRunResult(
                table_result=upsert_result.table_result,
                sync_run_id=sync_run_id,
            ),
            sync_run_id=sync_run_id,
            message="SP-API FBA Reimbursements ingestion completed and committed to Azure SQL.",
        )

    def _upsert_preview_rows(
        self,
        *,
        repo: FbaReimbursementsRepo,
        dry_run_result: FbaReimbursementsIngestionDryRunResult,
        source_run_id: int,
    ) -> FbaReimbursementsUpsertRunResult:
        preview_path = dry_run_result.report_result.preview_file_path
        rows = read_jsonl(preview_path) if preview_path else []
        table_result = repo.upsert_fba_reimbursement_rows(
            rows=rows,
            source_run_id=source_run_id,
        )
        return FbaReimbursementsUpsertRunResult(
    ...
    *,
    ...
    return {
        **dry_run_result.task_audit_event,
        "workflow_name": "sp_api_fba_reimbursements_ingestion",
        "job_name": "ingest_fba_reimbursements_report",
        ...
        "rows_skipped": 0,
        "rows_failed": 0,
        "message": "SP-API FBA Reimbursements ingestion is running.",
        "error_type": None,
        "error_detail": None,
    }
...
__all__ = [
    "FbaReimbursementsIngestionRunResult",
    "FbaReimbursementsIngestionService",
