...
from seller_data_pipeline.ingestion.settlement_table_mapping import read_jsonl


@dataclass(frozen=True)
class SettlementIngestionRunResult:
    workflow_name: str
    mode: str
    status: str
    requires_review: bool
    dry_run_result: SettlementIngestionDryRunResult
    upsert_result: SettlementUpsertRunResult | None
...
class SettlementIngestionService:
    """Guarded Settlement ingestion with dry-run default and explicit Azure SQL writes."""
    ...
    def __init__(
        ...
        *,
        ...
        self.dry_run_service = SettlementIngestionDryRunService(
    ...
    def run(
        ...
        *,
        ...
        if dry_run_result.requires_review:
            return SettlementIngestionRunResult(
                ...
                upsert_result=None,
                sync_run_id=None,
                message="Settlement ingestion requires schema review; Azure SQL write was blocked.",
            )
        if not execute:
            return SettlementIngestionRunResult(
                workflow_name="sp_api_settlement_ingestion",
        ...
        with get_connection() as conn:
            ...
            try:
                ...
                repo.insert_schema_validation_events(
                    list(dry_run_result.schema_validation_events),
                    source_run_id=sync_run_id,
                )
                finished_event = build_finished_audit_event(
                    ...
                    started_at=started_at,
                    status="success",
                    message="SP-API Settlement ingestion completed successfully.",
                )
                ...
                failed_event = build_finished_audit_event(
                    ...
                    started_at=started_at,
                    status="failed",
                    message="SP-API Settlement ingestion failed and was rolled back.",
                    error=exc,
                )
        ...
        return SettlementIngestionRunResult(
            ...
            requires_review=False,
            dry_run_result=dry_run_result,
            upsert_result=SettlementUpsertRunResult(
                table_result=upsert_result.table_result,
                sync_run_id=sync_run_id,
            ),
            sync_run_id=sync_run_id,
            message="SP-API Settlement ingestion completed and committed to Azure SQL.",
        )
    ...
    def _upsert_preview_rows(
        ...
        *,
        ...
        return SettlementUpsertRunResult(
    ...
    *,
    ...
    return {
        **dry_run_result.task_audit_event,
        ...
        "rows_skipped": 0,
        "rows_failed": 0,
        "message": "SP-API Settlement ingestion is running.",
        "error_type": None,
        "error_detail": None,
    }
