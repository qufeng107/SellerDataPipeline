...
from seller_data_pipeline.ingestion.sales_traffic_ingestion import SalesTrafficIngestionService


def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Run guarded SP-API Sales & Traffic ingestion. The default mode is "
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional explicit GET_SALES_AND_TRAFFIC_REPORT raw file. "
    ...
    service = SalesTrafficIngestionService(
    ...
    payload = result.to_dict() if len(results) == 1 else {
        "workflow_name": "sp_api_sales_traffic_period_ingestion",
    ...
    print(
        f"Sales & Traffic period ingestion files_processed={len(results)} failed={failed} "
    ...
    for item in results:
        print(f"Sales & Traffic ingestion mode={item.mode} status={item.status}")
        print(item.message)
