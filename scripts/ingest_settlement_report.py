...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Run guarded SP-API Settlement ingestion. The default mode is dry-run only; "
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional explicit GET_V2_SETTLEMENT_REPORT_DATA_FLAT_FILE_V2 raw file. "
            "May be supplied multiple times. If omitted, all files under "
            "reports/raw/amazon/{marketplace}/... are used."
        ),
    )
    parser.add_argument(
        "--output-root",
    ...
    service = SettlementIngestionService(
        raw_reports_root=settings.raw_reports_root,
        output_root=args.output_root,
    )
    result = service.run(
        marketplace_id=marketplace_id,
        raw_file_paths=args.raw_file,
    ...
    print(f"Settlement ingestion mode={result.mode} status={result.status}")
    ...
    print(
        "processed_files={files} skipped_files={skipped}".format(
            files=result.dry_run_result.processed_file_count,
            skipped=result.dry_run_result.skipped_file_count,
        )
    )
    if result.upsert_result is not None:
        ...
        print(
            "{table}: attempted={attempted} inserted={inserted} updated={updated} skipped={skipped}".format(
