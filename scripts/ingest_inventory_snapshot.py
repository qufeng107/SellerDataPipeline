...
from seller_data_pipeline.ingestion.inventory_ingestion import InventoryIngestionService


def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Run guarded SP-API FBA Inventory snapshot ingestion. The default mode is "
            "dry-run only; use --execute to write to Azure SQL after migrations have been applied."
        )
    )
    parser.add_argument(
        "--marketplace-id",
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional explicit GET_FBA_MYI_UNSUPPRESSED_INVENTORY_DATA raw file. "
    ...
    parser.add_argument(
        "--currency",
        default="USD",
        help="Currency stamped on inventory prices. Defaults to USD for US marketplace canary.",
    )
    parser.add_argument(
        "--output-root",
    ...
    service = InventoryIngestionService(
    ...
    print(f"Inventory ingestion mode={result.mode} status={result.status}")
