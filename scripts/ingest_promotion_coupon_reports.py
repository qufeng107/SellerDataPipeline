...
from seller_data_pipeline.ingestion.promotion_coupon_ingestion import (
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Run guarded SP-API Promotion/Coupon ingestion. "
    ...
    parser.add_argument(
        "--promotion-raw-file",
        default=None,
        help="Optional explicit GET_PROMOTION_PERFORMANCE_REPORT raw file.",
    )
    parser.add_argument(
        "--coupon-raw-file",
        default=None,
        help="Optional explicit GET_COUPON_PERFORMANCE_REPORT raw file.",
    )
    parser.add_argument(
        "--output-root",
    ...
    parser.add_argument(
        "--allow-review",
        action="store_true",
        help="Do not exit non-zero when schema review is required. Database writes are still blocked.",
    )
    parser.add_argument(
        "--json-output",
        default=None,
    ...
    service = PromotionCouponIngestionService(
        raw_reports_root=settings.raw_reports_root,
        output_root=args.output_root,
    )
    result = service.run(
        marketplace_id=marketplace_id,
        promotion_raw_file_path=args.promotion_raw_file,
        coupon_raw_file_path=args.coupon_raw_file,
    ...
    print(f"Promotion/Coupon ingestion mode={result.mode} status={result.status}")
