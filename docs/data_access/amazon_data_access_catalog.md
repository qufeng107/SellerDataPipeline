...
|---|---|---|---|---|
| SP-API Reports | `createReport` / `getReports` / `getReportDocument`；settlement 使用 Amazon-generated report discovery | TSV/TXT flat file 或 report-specific JSON | 多数第一阶段核心经营、库存、订单、财务、促销类报告已取样并生成脱敏字段记录 | [`sp_api_reports_catalog.md`](sp_api_reports_catalog.md) |
...
|---|---|---|---:|---|
| `GET_COUPON_PERFORMANCE_REPORT` | SP-API Reports | `json` | 2 | sampled + analyzed |
| `GET_FBA_ESTIMATED_FBA_FEES_TXT_DATA` | SP-API Reports | `delimited` | 8 | sampled + analyzed |
| `GET_FBA_INVENTORY_PLANNING_DATA` | SP-API Reports | `delimited` | 4 | sampled + analyzed |
| `GET_FBA_MYI_UNSUPPRESSED_INVENTORY_DATA` | SP-API Reports | `delimited` | 5 | sampled + analyzed |
| `GET_FBA_REIMBURSEMENTS_DATA` | SP-API Reports | `delimited` | 19 | sampled + analyzed |
| `GET_FLAT_FILE_ALL_ORDERS_DATA_BY_ORDER_DATE_GENERAL` | SP-API Reports | `delimited` | 112 | sampled + analyzed |
| `GET_FLAT_FILE_RETURNS_DATA_BY_RETURN_DATE` | SP-API Reports | `delimited` | 0 | sampled header-only / empty |
| `GET_LEDGER_DETAIL_VIEW_DATA` | SP-API Reports | `delimited` | 207 | sampled + analyzed |
| `GET_LEDGER_SUMMARY_VIEW_DATA` | SP-API Reports | `delimited` | 150 | sampled + analyzed |
| `GET_MERCHANT_LISTINGS_ALL_DATA` | SP-API Reports | `delimited` | 6 | sampled + analyzed |
| `GET_PROMOTION_PERFORMANCE_REPORT` | SP-API Reports | `json` | 1 | sampled + analyzed |
...
| `GET_SALES_AND_TRAFFIC_REPORT` | SP-API Reports | `json` | 6 | sampled + analyzed |
| `GET_V2_SETTLEMENT_REPORT_DATA_FLAT_FILE_V2` | SP-API Reports | `delimited` | 4911 | discovered + sampled aggregate |
...
这些文件是脱敏后的字段统计和样例分析结果，不是原始 Amazon 报表。后续迁移完成后，`docs/data_access/` 是正式目录；`requirements/data_samples/` 仍可作为字段取样记录的低层来源。
