...
如果未来使用 Azure Container Apps Jobs 做定时任务，不能依赖人工先运行一次 `test_azure_sql_connection.py`。自动化任务必须自己处理连接预热。
...
这些错误必须暴露出来，由对应功能的 transaction、幂等性和验收规则处理。
...
4. 若切换到 non-serverless Azure SQL 或改变 compute tier，本规则仍可保留；它只会增加一次轻量 `SELECT 1` warm-up。
